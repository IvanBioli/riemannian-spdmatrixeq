function [h, store] = lyap_psd_hess(X, eta, A, M, N, B, store, options)
% lyap_psd_hess - Computes the Hessian-vector product between the
% Riemannian or projected Euclidean Hessian and the tangent vector eta.
%
% Syntax:
%   [h, store] = lyap_psd_hess(X, eta, A, M, N, B, store, options)
%
% Inputs:
%   - X      : Current iterate
