...
for j = 1:length(cell_optsfirstord)
    ...
    if whatruns_firstord_(3)
        ...
        names{3+idx_shift} = name_prefix + cg_type + " + Prec. Rich. $\mathcal{P}^{(1)}X = AX+XA$ (Bartels Stewart)"+ name_postfix;
    end
    ...
    if whatruns_firstord_(4)
        ...
        names{4+idx_shift} = name_prefix + cg_type + " + Prec. Rich. $\mathcal{P}^{(1)}X = AX+XA$ fADI$\times" +num2str(kmin) + "$"+ name_postfix;
    end
    ...
    if whatruns_firstord_(5)
        ...
        names{5+idx_shift} = name_prefix + cg_type + " + Prec. Rich. $\mathcal{P}^{(1)}X = AX+XA$ fADI$\times" +num2str(k) + "$"+ name_postfix;
    end
    ...
    if whatruns_firstord_(6)
        ...
        names{6+idx_shift} = name_prefix + cg_type + " + $\mathcal{P}^{(1)}X = AX+XA$ (exact)" + name_postfix;
    end
    ...
    if whatruns_firstord_(7)
        ...
        names{7+idx_shift} = name_prefix + cg_type + " + $\mathcal{P}^{(1)}X = AX+XA$ tangADI$\times" +num2str(kmin) + "$"+ name_postfix;
    end
    ...
    if whatruns_firstord_(8)
        ...
        names{8+idx_shift} = name_prefix + cg_type + " + $\mathcal{P}^{(1)}X = AX+XA$ tangADI$\times" +num2str(k) + "$"+ name_postfix;
    end
    ...
    if whatruns_firstord_(9)
        ...
        names{9+idx_shift} = name_prefix + cg_type + " + Prec. Rich. $\mathcal{P}^{(2)}X = AXD+DXA$ (Bartels Stewart)" + name_postfix;
    end
    ...
    if whatruns_firstord_(10)
        ...
        names{10+idx_shift} = name_prefix + cg_type + " + Prec. Rich. $\mathcal{P}^{(2)}X = AXD+DXA$ fADI$\times" +num2str(kmin) + "$"+ name_postfix;
    end
    ...
    if whatruns_firstord_(11)
        ...
        names{11+idx_shift} = name_prefix + cg_type + " + Prec. Rich. $\mathcal{P}^{(2)}X = AXD+DXA$ fADI$\times" +num2str(k) + "$"+ name_postfix;
    end
    ...
    if whatruns_firstord_(12)
        ...
        names{12+idx_shift} = name_prefix + cg_type + " + $\mathcal{P}^{(2)}X = AXD+DXA$ (exact)" + name_postfix;
    end
    ...
    if whatruns_firstord_(13)
        ...
        names{13+idx_shift} = name_prefix + cg_type + " + $\mathcal{P}^{(2)}X = AXD+DXA$ tangADI$\times" +num2str(kmin) + "$"+ name_postfix;
    end
    ...
    if whatruns_firstord_(14)
        ...
        names{14+idx_shift} = name_prefix + cg_type + " + $\mathcal{P}^{(2)}X = AXD+DXA$ tangADI$\times" +num2str(k) + "$"+ name_postfix;
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% RTR %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for j = 1:length(cell_optsrtr)
    ...
    if whatruns_rtr_(3)
        ...
        info = add_time(info, time_shifts_T);
        results{3+idx_shift} = info;
        names{3+idx_shift} = name_prefix + rtr_type + " + $\mathcal{P}^{(1)}X = AX+XA$ tangADI$\times" +num2str(k) + "$"+ name_postfix;
    end
    ...
    if whatruns_rtr_(4)
        ...
        results{4+idx_shift} = info;
        names{4+idx_shift} = name_prefix + rtr_type + " + $\mathcal{P}^{(2)}X = AXD+DXA$ (exact)" + name_postfix;
    end
    ...
    if whatruns_rtr_(5)
        ...
        info = add_time(info, time_shifts_T);
        results{5+idx_shift} = info;
        names{5+idx_shift} = name_prefix + rtr_type + " + $\mathcal{P}^{(2)}X = AXD+DXA$ tangADI$\times" +num2str(k) + "$"+ name_postfix;
    end
end
...
if sanity_checks
    ...
    names{length(whatruns)+2} = "Full rank pGD $\mathcal{P}^{(1)}X = AX+XA$";
    
    % Preconditioned steepest descent
    problem_check.precon =@(X,H) bartelsStewart(A_, D_, E_, B_, H, false, false);
    [~, ~, info, ~] = conjugategradient(problem_check, X0, opts_rgd);
    results{length(whatruns)+3} = info;
    names{length(whatruns)+3} = "Full rank pGD $\mathcal{P}^{(2)}X = AXD+DXA$";
end
%% Plot comparison
...
if plot_and_save
    ...
    savename = "pdes_FD_n" + num2str(n) + "_a" + num2str(alpha);
    if rank_adaptivity
