...
for i=1:length(adi_iterations_vec)
    ...
    % Run pCG with preconditioner P(X) = AXD+DXA
    ...
    results{2*i+1} = info;
    names{2*i+1} = "CG trunc. +" + cap_legend + " $\mathcal{P}^{(2)} X =AXD+DXA$ fADI$\times " + num2str(adi_niter) + "$";
end
...
if sanity_checks
    ...
    [~, info_fullrank] = pcg_trunc_lr(A_op_fullrank, M_inv_op_fullrank, ...
        full(C.L * C.R'), T_op_fullrank, options_fullrank);
    results{length(results)+1} = info_fullrank;
    names{length(names)+1} = "CG (full rank) + $\mathcal{P}^{(2)} X = AXD+DXA$ (exact)";
end

%% PLOTS
titletext = "$n = " + n + "$, $a =" + alpha + "$";
savename = rank_cap_str + "pdesFD_n" + num2str(n) + "_a" + num2str(alpha);
